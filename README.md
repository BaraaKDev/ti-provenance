...
Three skills, each self-contained. None of them knows the others exist — they meet through a JSON
contract, so you can run any one on its own:
...
`scripts/Test-Pipeline.ps1` runs the lot — 54 checks covering the documented invocation forms,
every refusal path, contract drift between the three skill copies, and a full PDF round trip.
...
A few environment notes, since they shaped the tooling:

- **Windows PowerShell 5.1**, and no Python on the target machine — so everything is PowerShell
  with no third-party modules.
